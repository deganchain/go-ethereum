...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"dbn": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + dbn.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + dbn.blockNumber + " (" + new Date(1000 * dbn.getBlock(dbn.blockNumber).timestamp) + ")\n";
		try {
